...
# Methods
The following code is apply a moving average function to a data frame and insert calculation into a new column called "chemical name"_moving_avg. 
...
chem_site_plot <- long_sites_avg %>% 
  ...
  geom_line(size = 1) +
  scale_color_discrete(labels = c("PMR", "BQ1", "BQ2", "BQ3")) +
  facet_grid(rows = vars(factor(chemical,
                     levels=c("k_moving_avg", "no3_n_moving_avg", "mg_moving_avg", "ca_moving_avg", "nh4_n_moving_avg"))), scales = "free", switch = "y", labeller=chemical_labeller) + 
  theme_classic() + 
  theme(legend.position = c(1.07, 0.9), 
  ...
  geom_vline(xintercept = as.Date("1989-09-19"), linetype = 3)
